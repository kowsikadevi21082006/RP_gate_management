...
FOR EACH ROW
BEGIN
    UPDATE personnel SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
    WHERE personnel_id = OLD.personnel_id;
END;

CREATE TRIGGER movement_updated_at
AFTER UPDATE ON movement_records
FOR EACH ROW
BEGIN
    UPDATE movement_records SET updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
